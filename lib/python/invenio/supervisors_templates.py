...
from invenio.supervisors_engine import get_invenio_error_details

#TP: sam pridal
from invenio.webuser import list_registered_users, get_user_info, get_email
from invenio.access_control_admin import acc_get_user_roles_from_user_info, acc_get_role_name, acc_find_possible_actions_user
from invenio.supervisors_config import cfg_supervisors_groups
from invenio.search_engine import search_pattern, search_pattern_parenthesised
...
class Template:
    ...
    def tmpl_welcome(self, ln=CFG_SITE_LANG):
        ...
        """
        return """<p>On these pages, you can review the overall progress of "Structuration" process and view the performance statistics of individual users.</p>"""
...
#            &timespan=last+month&s_date=01%2F29%2F2013+00%3A00&f_date=01%2F29%2F2013+09%3A28&format=flot&ids=cardStructured&cols0=UID&col_value0=tomas&action_gen=Generate
    ...
    def tmpl_display_numeric_stats(self, titles, avgs, maxs, mins, total=123):
        ...
        if titles:
            ...
            for i in range(len(titles)):
                out += """<em>%s</em><br />
                          ...
                          <b>Average:</b> %d<br />
                          <b>Max:</b> %d<br />
                          <b>Min:</b> %d<br />""" % (cgi.escape(titles[i]), total[i],
                                                    avgs[i], maxs[i], mins[i])
            return out
        else:
            return """<b>TOTAL:</b> %d<br />
                      <b>Average:</b> %d<br />
                      <b>Max:</b> %d<br />
                      <b>Min:</b> %d<br />""" % (total, avgs, maxs, mins)
