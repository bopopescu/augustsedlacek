...
def _upload_file_with_bibupload(file_path, upload_mode, num_records, req):
    ...
    if num_records < CFG_BIBEDITMULTI_LIMIT_INSTANT_PROCESSING:
        task_low_level_submission('bibupload', 'multiedit', '-P', '5', '-N', 'multiedit', upload_mode, '%s' % file_path) #TP: added specName
        return (0, file_path)
    elif num_records < CFG_BIBEDITMULTI_LIMIT_DELAYED_PROCESSING:
        task_low_level_submission('bibupload', 'multiedit', '-P', '5', '-N', 'multiedit', upload_mode, '-L', CFG_BIBEDITMULTI_LIMIT_DELAYED_PROCESSING_TIME,'%s' % file_path) #TP: added specName
        return (1, file_path)
    else:
