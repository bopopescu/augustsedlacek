...
def perform_request_init(uid, ln, req, lastupdated):
    ...
    protected_fields = ['001','040','970','980']
...
def perform_request_ajax(req, recid, uid, data, isBulk = False, \
    ...
    elif request_type in ('getRecord', 'submit', 'mergeRecord', 'cancel', 'newRecord',
    ...
    elif request_type in ('addNextCardImages', ): #TP: added
        response.update(perform_add_next_card_images(request_type, recid, uid, data))
...
def perform_request_record(req, request_type, recid, uid, data, ln=CFG_SITE_LANG):
    ...
    elif request_type == 'submit' or request_type == 'mergeRecord': #TP: puvodne jen submit
...
#                disabled_changes = tmp_result[4]
                ...
                if errors_upload:
                    ...
                    if request_type == "mergeRecord": #TP: puvodne jen save_xml_record(recid, uid) a yadnz if
                        save_xml_record(recid, uid, xml_record='', to_upload=True, to_merge=False, spec_name='bibedit_merge')
                    else:
        ...
        save_xml_record(recid, uid, revision_xml, spec_name='bibedit_revert') #TP: puvodne jen save_xml_record(recid, uid, revision_xml) 
...
def perform_request_autocomplete(request_type, recid, uid, data):
    ...
    if (request_type == 'autosuggest'):
        ...
        if fulltag and searchby and len(searchby) > 0: #TP: originally 3, for augustsedlacek 0 will be safe too
...
def perform_add_next_card_images(request_type, recid, uid, data): #TP:added
    """ Handle request to attach additional images from following card images
    ...
    currUrls = record_get_field_values(record, tag="956", ind1="4", ind2=" ", code="u")

    newImgAdded = False
    nextRecId = recid + len(currUrls)/2
    nextRecord = get_bibrecord(nextRecId)
    nextUrls = record_get_field_values(nextRecord, tag="956", ind1="4", ind2=" ", code="u")
    nextLabels = record_get_field_values(nextRecord, tag="956", ind1="4", ind2=" ", code="y")

#    if (len(nextUrls) == 2) and (uid == 1):
    if len(nextUrls) == 2:
        response['newImg1_url'] = nextUrls[0]   
        response['newImg2_url'] = nextUrls[1]
        response['newImg1_label'] = nextLabels[0]
        response['newImg2_label'] = nextLabels[1]
#        response['html_preview'] = _get_formated_record(record, data['new_window'])
        response['html_preview'] = "<br /><h2>Current set of card images:</h2>"
        for url in currUrls:
            response['html_preview'] += "<img class='imgLink2' src='/datastorage/" + url + "'/>"
        response['html_preview'] += "<br /><h2>Images of the next card to be considered for appending:</h2>"
        response['html_preview'] += "<a onclick='onclickPopupImage(this.title)' title='/datastorage/" + nextUrls[0] + "' style='cursor:pointer'><img class='imgLink3' src='/datastorage/" + nextUrls[0] + "'/></a>"
        response['html_preview'] += "<a onclick='onclickPopupImage(this.title)' title='/datastorage/" + nextUrls[1] + "' style='cursor:pointer'><img class='imgLink3' src='/datastorage/" + nextUrls[1] + "'/></a>"
    else:
        response['html_preview'] = "<br /><h2>Neznama chyba. Prosim, stisknete ESC.</h2>" % uid
    return response
