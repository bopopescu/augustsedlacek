...
from invenio.supervisors_config import CFG_WEBSTAT_CONFIG_PATH
...
from invenio.supervisors_engine import get_keyevent_trend_collection_population, \
...
from invenio.supervisors_engine import get_customevent_table, \
    get_customevent_trend, \
    get_customevent_dump

# Imports handling custom report
from invenio.supervisors_engine import get_custom_summary_data, \
    _get_tag_name, \
    create_custom_summary_graph

# Imports for handling outputting
from invenio.supervisors_engine import create_graph_trend, \
    create_graph_dump, \
    create_graph_table, \
    get_numeric_stats

# Imports for handling exports
from invenio.supervisors_engine import export_to_python, \
    export_to_csv, \
    export_to_file

TEMPLATES = template.load('supervisors')
...
def perform_request_index(ln=CFG_SITE_LANG):
    ...
    out += TEMPLATES.tmpl_progress_overview(ln=ln)

    out += TEMPLATES.tmpl_user_list(ln=ln)
...
def _perform_display_event(data, name, settings, ln=CFG_SITE_LANG):
    ...
    if settings["format"] == "asciiart":
        ...
        if settings["format"] == "gnuplot":
            ...
            out = TEMPLATES.tmpl_display_event_trend_ascii(
                    settings["title"], path, ln=ln)
    avgs, maxs, mins, total = get_numeric_stats(data, settings["multiple"] is not None)
    return out + TEMPLATES.tmpl_display_numeric_stats(settings["multiple"],
                    avgs, maxs, mins, total)
