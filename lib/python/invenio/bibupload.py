...
def bibupload(record, opt_tag=None, opt_mode=None,
    ...
    try:
        if not record_is_valid(record, pretend=pretend): #TR: pridan parametr pretend
            #TP: msg = "ERROR: record is not valid (%s %s %s %s). " % (opt_mode, task_get_task_param('task_id', 0), task_get_task_param('task_name', 0),task_get_task_param('task_specific_name', 0)) #TP: added opt_mode and other
...
#    register_customevent("cardStructured", ["novy8", "KUA", "posledni"])

    rec_ids = record_get_field_values(record, tag="001")
    if len(rec_ids) != 1:
        write_message("    The record is not valid: it has not a single rec_id: %s" % (len(rec_ids)), stream=sys.stderr)
        return False

#    from invenio.supervisors import register_customevent2
#    write_message("Test %s<br/>" % register_customevent("kukuc","testxx1","testxx2","testxx3"), stream=sys.stderr)
#    register_customevent("cardStructured",task_get_task_param('user'),"A01A",rec_ids)
#    register_customevent('cardStructured',('TEST','A01A','rec_ids'))
#    write_message("Test %s<br/>" % register_customevent2('cardStructured',["TEST","A01A","rec_ids"]), stream=sys.stderr)
#    register_customevent("baskets", ["display", "", "user_str"])
#    import invenio.dbquery
#    run_sql("INSERT INTO staEVENT09 (`user_id`,`time`,`recid`) VALUES (`TEST`, `A01A`, `rec_ids`)");

    rec_status = record_get_field_values(record, tag="950", ind1="%", ind2="%", code="s")
    if task_get_task_param('task_specific_name') not in ['bibedit_autoupdate', 'multiedit', 'bibedit_revert', 'bibedit_merge', 'bibupload_initialUpload']:
        count_190 = record_count_fields(record, tag="190")
        if count_190 > 1:
            write_message("    Neplatný záznam - pole 'Zahlavi - Kartotecni zaznam' smí být uvedeno pouze jednou.<br/><br/>", stream=sys.stderr)
            return False
        if count_190 == 0:
        ...
        count_670 = record_count_fields(record, tag="670")
        if count_670 < 1:
            write_message("    Neplatný záznam - pole 'Citace' musí být uvedeno.<br/><br/>", stream=sys.stderr)
            return False

        for field in record["670"]:
            subs=0
            for subfield in field[0]:
                ...
                if subfield[0] == "z":
                    subs = subs+1
            if subs != 1:
                write_message("    Neplatný záznam - chyba v poli 'Citace'. Z následujících podpolí je nutno uvést jedno a pouze jedno: 'Citace', 'Citace - zkratka', 'Interní revokace'.<br/><br/>", stream=sys.stderr)
                return False
        for field in record["670"]:
            sub_9=0
            for subfield in field[0]:
                if subfield[0] == "9":
                    sub_9 = sub_9+1
            if sub_9 > 1:
                write_message("    Neplatný záznam - chyba v poli 'Citace'. Podpole 'Typ zdrojoveho dokumentu' musí být uvedeno nejvýše jednou.<br/><br/>", stream=sys.stderr)
                return False

        rec_950p = record_get_field_values(record, tag="950", ind1="%", ind2="%", code="p")
        if len(rec_950p) != 1:
            write_message("    Neplatný záznam - je nutno uvést informaci 'Pokračující lístky' v poli 'Status' (jen jeden výskyt).<br/><br/>", stream=sys.stderr)
            return False
        count_956 = record_count_fields(record, tag="956")
        if count_956 < 2:
            write_message("    Neplatný záznam - chyba v poli 'Obrazek'. Chybný počet polí.<br/><br/>", stream=sys.stderr)
            return False

        elif count_956 > 2 and rec_950p[0] not in ["1","pokračující lístek"]:
            write_message("    Neplatný záznam - v záznamu nalezeno více obrázků, čemuž ale neodpovídá hodnota v poli 'Pokračující lístky'.<br/><br/>", stream=sys.stderr)
            return False


        for field in record["956"]:
            sub_u=0
            sub_y=0
            sub_1=0
            for subfield in field[0]:
                ...
                if subfield[0] == "1":
                    sub_1 = sub_1+1
            if sub_u != 1:
                write_message("    Neplatný záznam - chyba v poli 'Obrazek'. Podpole 'URL' musí být uvedeno právě jednou.<br/><br/>", stream=sys.stderr)
                return False
            if sub_y != 1:
                write_message("    Neplatný záznam - chyba v poli 'Obrazek'. Podpole 'Nazev obrazku' musí být uvedeno právě jednou. <br/><br/>", stream=sys.stderr)
                return False
            if sub_1 != 1:
                write_message("    Neplatný záznam - chyba v poli 'Obrazek'. Podpole 'Format listku' musí být uvedeno právě jednou. <br/><br/>", stream=sys.stderr)
                return False

        count_950 = record_count_fields(record, tag="950")
        if count_950 > 1:
            write_message("    Neplatný záznam - pole 'Status' smí být uvedeno pouze jednou.<br/><br/>", stream=sys.stderr)
            return False
        if count_950 == 0:
            write_message("    Neplatný záznam - pole 'Status' chybí.<br/><br/>", stream=sys.stderr)
            return False

        for field in record["950"]:
            sub_p=0
            sub_s=0
            for subfield in field[0]:
                if subfield[0] == "p":
                ...
                if subfield[0] == "s":
                    sub_s = sub_s+1
            if sub_p != 1:
                write_message("    Neplatný záznam - chyba v poli 'Status'. Podpole 'Pokracujici listky' musí být uvedeno právě jednou.<br/><br/>", stream=sys.stderr)
                return False
            if sub_s != 1:
                write_message("    Neplatný záznam - chyba v poli 'Status'. Podpole 'Status' musí být uvedeno právě jednou. <br/><br/>", stream=sys.stderr)
                return False


        # testing date formats
        for tag in ["190","400","500","700","711","751"]:
            if tag in record:
                for field in record[tag]:
                    for subfield in field[0]:
                        if subfield[0] == "y":
                            result = field_check_date(subfield[1])
                            if result != "OK":
                                write_message("    Neplatný záznam - chybne zadana datace. Dataci zadavejte ve tvaru 'DD.MM.RRRR', 'MM.RRRR', 'RRRR','DD.MM.RRRR-DD.MM.RRRR', 'MM.RRRR-MM.RRRR', nebo 'RRRR-RRRR'.<br/><br/>", stream=sys.stderr)
                                return False

        # testing non-mandatory names
        for tag in ["400","500","700"]:
            if tag in record:
                for field in record[tag]:
                    count=0
                    for subfield in field[0]:
                        if subfield[0] == "a" or subfield[0] == "1" or subfield[0] == "3" or subfield[0] == "4": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - pokud je uvedeno pole se jmenem, musi by uvedeno nejktere z jeho podpoli 'Osobni jmeno' nebo 'Prijmi / Prijmeni' nebo 'Predikat' nebo 'Domicil'.<br/><br/>", stream=sys.stderr)
                        return False
        # testing non-mandatory names
        for tag in ["667"]:
            if tag in record:
                for field in record[tag]:
                    ...
                    for subfield in field[0]:
                        if subfield[0] == "a": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - v poli 'Neverejna poznamka' musi by uvedeno podpole 'Neverejna poznamka'.<br/><br/>", stream=sys.stderr)
                        return False
        # testing non-mandatory field/subfield
        for tag in ["668"]:
            if tag in record:
                for field in record[tag]:
                    count=0
                    for subfield in field[0]:
                        if subfield[0] == "a": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - v poli 'Biografie' musi by uvedeno podpole 'Biografie'.<br/><br/>", stream=sys.stderr)
                        return False
        # testing non-mandatory field/subfield
        for tag in ["680"]:
            if tag in record:
                for field in record[tag]:
                    ...
                    for subfield in field[0]:
                        if subfield[0] == "i": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - v poli 'Poznamky' musi by uvedeno podpole 'Poznamky'.<br/><br/>", stream=sys.stderr)
                        return False
        # testing non-mandatory field/subfield
        for tag in ["711"]:
            if tag in record:
                for field in record[tag]:
                    ...
                    for subfield in field[0]:
                        if subfield[0] == "a": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - v poli 'Udalost - propojeni' musi by uvedeno podpole 'Udalost'.<br/><br/>", stream=sys.stderr)
                        return False
        # testing non-mandatory field/subfield
        for tag in ["751"]:
            if tag in record:
                for field in record[tag]:
                    ...
                    for subfield in field[0]:
                        if subfield[0] == "a": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - v poli 'Mistni nazev - propojeni' musi by uvedeno podpole 'Mistni nazev'.<br/><br/>", stream=sys.stderr)
                        return False
        # testing non-mandatory field/subfield
        for tag in ["690"]:
            if tag in record:
                for field in record[tag]:
                    count=0
                    for subfield in field[0]:
                        if subfield[0] == "a" or subfield[0] == "g": 
                            count = count+1
                    if count == 0:
                        write_message("    Neplatný záznam - pokud je uvedeno pole 'Heraldicke udaje', musi by uvedeno nejktere z jeho podpoli 'Heraldicke udaje' nebo 'Typ grafickeho prvku'.<br/><br/>", stream=sys.stderr)
                        return False

        #passed - add auto fields
        tt = ("a",time.strftime("%Y-%m-%d %H:%M:%S")),("b",task_get_task_param('user'))
#        write_message("TEST: %s" % len(tt), stream=sys.stderr)
        testik = record_add_field(record, "906", ind1=' ', ind2=' ', subfields=tt)
#        write_message("TEST: %s" % testik, stream=sys.stderr)

        #set 950 of the following cards if needed
        if count_956>2:
            cid = int(rec_ids[0])
            lid = cid - 1 + count_956/2
#            write_message("    Vse bylo ok, jdu nastavit priznamy v dalsich zaznamech.<br/>cid=%s<br/>lid=%s<br/>" % (cid, lid), stream=sys.stderr)
            from invenio.bibedit_utils import get_bibrecord, save_xml_record
            while cid<lid:
#                write_message("<br/>menim id=%s<br/>" % lid, stream=sys.stderr)
                uRecord = get_bibrecord(lid)
                newStatus = (("s","přepsané"),("p","připojený k předchozímu"))
                record_delete_field(uRecord, "950", ind1=' ', ind2=' ')
                record_add_field(uRecord, "950", ind1=' ', ind2=' ', subfields=newStatus)
                record_add_field(uRecord, "906", ind1=' ', ind2=' ', subfields=tt)
#                update_cache_file_contents()
                save_xml_record(lid, 0, record_xml_output(uRecord), spec_name='bibedit_autoupdate')
#                write_message("XML: %s<br/>" % record_xml_output(uRecord), stream=sys.stderr)
                lid = lid - 1
...
def field_check_date(d):
    ...
    error = 0
    for md in mdate:
#        write_message("test: %s <br/>" % md, stream=sys.stderr)
        for format in ['%d.%m.%Y','%m.%Y','%Y']:
          try:
            result = datetime.strptime(md, format)
          except:
#            write_message("except for %s <br/>" % error, stream=sys.stderr)
            pass
          else:
            error = error + 1
#            write_message("passed for %s<br/>" % error, stream=sys.stderr)

#    write_message("finished with %s<br/>" % error, stream=sys.stderr)
#    write_message("len %s<br/>" % len(mdate), stream=sys.stderr)

    if error != len(mdate):
#      write_message("neni %s<br/>" % error, stream=sys.stderr)
      return "KO"
    else:
#      write_message("OK<br/>", stream=sys.stderr)
      return "OK"

def record_count_fields(rec, tag):
    """
    TP: simple record counting, without wildcards and regardless of ind values
    """
    tmp = 0
