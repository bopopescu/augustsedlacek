...
def perform_request_ajax(req, uid, data):
    ...
    #TP: pridan mergeRecord
    if requestType in ('getRecordCompare', 'submit', 'mergeRecord', 'cancel', 'recCopy', \
           'recMerge', 'recMergeNC'):
...
def perform_request_record(requestType, uid, data):
    ...
    if requestType == 'submit' or requestType == 'mergeRecord': #TP: pridan  requestType == 'mergeRecord'
        spec_name = ''
        if requestType == 'mergeRecord':
            spec_name = 'bibedit_merge'
        if data.has_key('duplicate'):
            ...
            save_xml_record(recid1, uid, xml_record1, to_upload=True, to_merge=False, spec_name=spec_name) #TP: puvodne            save_xml_record(recid1, uid, xml_record1)
            #submit record2
            xml_record2 = record_xml_output(record2)
            save_xml_record(recid1, uid, xml_record1, to_upload=True, to_merge=False, spec_name=spec_name) #TP: puvodne            save_xml_record(recid2, uid, xml_record2)

            result['resultText'] = 'Records submitted'
            return result

        #submit record1
        save_xml_record(recid1, uid, spec_name=spec_name)
