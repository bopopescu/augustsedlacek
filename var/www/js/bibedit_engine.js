...
this has been added by SK and later TP:
*    - drawSeelectSubfieldValues - TP
*    - drawField_856_ImagesThumbs - SK/TP
*    - checkVersionNumbers - TP
*    - doplnit
...
$(function(){
  ...
  // USER SCRIPTS
    console.log(gTAG_NAMES);
  $('#ImgHoldingPenMenu, #btnSwitchReadOnly').parent().parent().hide();
  $('#bibEditBibCircConnection').hide();

  var fieldTemplatesCounter = fieldTemplates.length;
  for(var key in userFieldTemplates) {
    fieldTemplates[parseInt(key)+fieldTemplatesCounter] = userFieldTemplates[key]
  }

      //  console.log(userFieldTemplates);
  //fieldTemplates[5] = userFieldTemplates[5];
      //  console.log(fieldTemplates);
  domChagesListener();
  //$('#bibEditContent').
});

function domChagesListener() {
  var intervalID = setInterval(function() { 
    if ($('#bibEditTable').length) {
//      console.log('Y');
...
function setIventListeners() {
  $('#bibEditContent').on('click', '.imgLink', function(){
    var image = new Image();
    image.src = $(this).attr('href');
    var w=600;
    var h=400;

var newwindow= window.open('', 'newwindow', 'width='+w+',height='+h+',top=100,left=500,scrollbars=1');
if(newwindow.location == 'about:blank' ){
    newwindow.location = $(this).attr('href');

    newwindow.onresize=function(){
        var angle = 0;
if (this.document.getElementById('mycan')) {

  var p =this.document.getElementById('mycan').oImage;

  var n = p.src.indexOf('datastorage/');
  var result = p.src.substring(n + 12);
  var angleElm = $("td:contains("+result+")").parent().parent().find("td:contains('Zpusob natoceni')").next();
  if (angleElm) angle = 1*angleElm.text();

        //TP: see http://code.google.com/p/jquery-rotate/
        ...
        if (document.all && !window.opera) {
          var canvas = this.document.getElementById('mycan');
          ...
          if (angle==90 || angle==270) {
            canvas.oImage.height = this.window.innerWidth;
            canvas.oImage.width = Math.abs(orh*this.window.innerWidth/orw);
          } else {
            canvas.oImage.width = this.window.innerWidth;
            canvas.oImage.height = Math.abs(orh*this.window.innerWidth/orw);
          }
...
//        p.parentNode.replaceChild(canvas, p);
      ...
      newwindow.onload=function(){
        var angle = 0;
        var p =this.document.getElementsByTagName('img')[0];
 ...
 //     alert('je otevreno.. je tam '+newwindow.document.getElementById('mycan').oImage.src);
      newwindow.document.getElementById('mycan').oImage.src=image.src;
//      alert('je otevreno.. zmeneo na '+newwindow.document.getElementById('mycan').oImage.src);



        var angle = 0;
        var p =newwindow.document.getElementById('mycan').oImage;
  ...
  if (angleElm) angle = 1*angleElm.text();
        ...
        if (document.all && !window.opera) {
          var canvas = newwindow.document.getElementById('mycan');
          ...
          if (angle==90 || angle==270) {
            canvas.oImage.height = newwindow.window.innerWidth;
            canvas.oImage.width = Math.abs(orh*newwindow.window.innerWidth/orw);
          } else {
            canvas.oImage.width = newwindow.window.innerWidth;
            canvas.oImage.height = Math.abs(orh*newwindow.window.innerWidth/orw);
          }
...
//        p.parentNode.replaceChild(canvas, p);
  ...
  $('#bibEditContent').on('change', '.f_selectbox_wrapper select', function(){
    var tagName = $(this).val();
    $(this).parent().parent().find('#txtAddFieldTag_'+this.id.substr(18)).val(tagName);
    $(this).parent().parent().parent().parent().find('.sf_selectbox_wrapper select').each(function(){
      $(this).parent().html(select(this.id, chooseSubfieldsSet(tagName), '0'));
    });
    $(this).parent().parent().parent().parent().find('.bibEditTxtSubfieldCode').each(function(){
      $(this).val('');
    });
    $(this).parent().parent().parent().parent().find('.value_selectbox_wrapper select').each(function(){
      $(this).parent().html(select(this.id, {}, '0')).hide();   
    });
    $(this).parent().parent().parent().parent().find('.bibEditTxtValue').each(function(){
      $(this).val('').removeAttr('disabled');
    });    
    //console.log(this.id.substr(18));
  });
  $('#bibEditContent').on('change', '.sf_selectbox_wrapper select', function(){
    $(this).parent().parent().find('.bibEditTxtSubfieldCode').val($(this).val());
    var f_val = $(this).parent().parent().parent().parent().find('input.bibEditTxtTag').val();
    if (f_val === undefined) f_val = $(this).parent().parent().parent().parent().find('.bibEditCellFieldTag').attr('id').substr(9,3);
    var subfieldValues = getSubfieldValues(f_val.toString()+$(this).val().toString());
    var value_selectbox_wrapper_style = ' style="display:none;"';
    var subfieldValueAttrs = {value : ''};
    //console.log(f_val.toString()+$(this).val().toString());
    if (subfieldValues !== undefined) {
      //subfieldValueAttrs['disabled'] = 'disabled';
      value_selectbox_wrapper_style = '';
      $(this).parent().parent().parent().find('.value_selectbox_wrapper select').each(function(){
        $(this).parent().html(select(this.id, subfieldValues, '0')).show();
      });
      $(this).parent().parent().parent().find('.bibEditTxtValue').each(function(){
  ...
  $('#bibEditContent').on('change', '.value_selectbox_wrapper select', function(){
    //TP: puvodne jen    $(this).parent().parent().find('.bibEditTxtValue').val($(this).val());
    var relTxt = $(this).parent().parent().find('.bibEditTxtValue');
    relTxt.val($(this).val());
    var temp=$(this).attr("id").substring($(this).attr("id").indexOf("_") + 1);
    var cTag=temp.substring(0,temp.indexOf("_"));
    var cPos=temp.substring(temp.indexOf("_")+1);
    relTxt.removeClass("bibEditVolatileSubfield");
    onAddFieldJumpToNextSubfield("#rowGroupAddField_"+cTag,cTag, cPos);
  });

  /*
   * selectbox for existing subfield content
  $('#bibEditContent tr').each(function() {
    var tmpArray = this.id.split('_');
    if (tmpArray[3] != undefined) {
      var tag = tmpArray[1];
      var fieldPosition = tmpArray[2];
      var subfieldIndex = tmpArray[3];
      var field = gRecord[tag][fieldPosition];
      var subfield = field[0][subfieldIndex][0];
      var subfieldValues = getSubfieldValues(tag.toString()+subfield.toString());
      if (subfieldValues !== undefined) {
        var selectbox = '<tr class="selectbox_ext"><td></td><td></td><td></td><td></td><td id="contentSelectBox_'+tag+'_'+fieldPosition+'_'+subfieldIndex+'" class="value_selectbox_wrapper_external">'+select('selectAddValue_' + tag+'_'+fieldPosition+'_'+subfieldIndex, subfieldValues, '0') + '</td><td></td>';
        $(this).before(selectbox);
      }
    }
  });
   */
  $('#bibEditContent').on('change', '.value_selectbox_wrapper_external select', function() {
    var tmpArray = this.id.split('_');
    if (tmpArray[3] != undefined) {
      var tag = tmpArray[1];
      var fieldPosition = tmpArray[2];
      ...
      var value = $(this).val();
      //console.log(value);
      var tempVal = gRecord[tag][fieldPosition][0][subfieldIndex][1];
      gRecord[tag][fieldPosition][0][subfieldIndex][1] = value;
      $(this).parent().parent().parent().find('#content_'+this.id.substr(15)).click();
      gRecord[tag][fieldPosition][0][subfieldIndex][1] = tempVal;
    }
  });
}
...
function filterChanges(changeset){
  ...
  unchangableTags = {"001" : true,"040" : true,"970" : true,"980" : true}; // a dictionary of the fields that should not be modified
  result = [];
...
function onAddNextCardImagesSuccess(dialogPreview, html_preview, resp){ //TP: added
  /*
   * Confirm whether to submit the record
   *
   * dialog: object containing the different parts of the modal dialog
  ...
  */
  ...
  //get related next Field Values

  addContentToDialog(dialogPreview, html_preview, "Do you want to attach running card images?");
  dialogPreview.dialogDiv.dialog({
        title: "Verify running cards",
        close: function() { updateStatus('ready'); },
        buttons: {
            "Attach the images": function() {
/*                                    createReq({recID: gRecID, requestType: 'submit',
                                        force: onSubmitClick.force}, function(json){
                                            // Submission was successful.
                                            changeAndSerializeHash({state: 'submit', recid: gRecID});
...
*/
              ...
              adNewImageField(resp['newImg1_url'],resp['newImg1_label']);
              adNewImageField(resp['newImg2_url'],resp['newImg2_label']);
              draw_ImagesThumbs(resp['newImg1_url'],resp['newImg1_label']);
              draw_ImagesThumbs(resp['newImg2_url'],resp['newImg2_label']);
              $( this ).remove();
                                },
            "Cancel - DON'T attach the Images": function() {
                        updateStatus('ready');
                        $( this ).remove();
                    }
    }});
...
function onNextCardImagesClick(){ //TP: added
  /*
   * Handle 'Submit' button (submit record).
   */
  updateStatus('updating');
  ...
  // Get preview of the record and let the user confirm submit
  addNextCardImages(dialogPreview, onAddNextCardImagesSuccess);
}
...
function addNextCardImages(dialog, onSuccess) { //TP: added
    var html_preview;
    var resp = new Array;
    createReq({'new_window': false, recID: gRecID, requestType: 'addNextCardImages'
       }, function(json){
       // Preview was successful.
        html_preview = json['html_preview'];
        resp['nextRecToBeUpdated_id'] = json['nextRecToBeUpdated_id'];
        resp['newImg1_url'] = json['newImg1_url'];
        resp['newImg2_url'] = json['newImg2_url'];
        resp['newImg1_label'] = json['newImg1_label'];
        resp['newImg2_label'] = json['newImg2_label'];
        onSuccess(dialog, html_preview, resp);
       });
}
...
function addFieldAddSubfieldEditor(jQRowGroupID, fieldTmpNo, defaultCode, defaultValue, tagName){
  /**
  ...
  */
  ...
  $(addFieldRows).eq(addFieldRows.length-1).before(createAddFieldRow(
    fieldTmpNo, subfieldTmpNo, defaultCode, defaultValue, '', '', '', tagName));
  ...
  */
  ...
  else
  {
    ...
    for (subfieldInd in formData.subfields){
      ...
      addFieldAddSubfieldEditor(jQRowGroupID, fieldTmpNo, subfield[0], subfield[1], formData.tag);
    }

    // now changing the main field properties
    console.log('1 == '+formData.tag);
    (formData.tag != '') 
      ? $('#rowGroupAddField_100000 .f_selectbox_wrapper').hide()
      : $('#rowGroupAddField_100000 .f_selectbox_wrapper').show();
    $("#txtAddFieldTag_" + fieldTmpNo).attr("value", formData.tag);
...
function createAddFieldInterface(initialContent, initialTemplateNo){
  ...
  $('#btnAddFieldAddSubfield_' + fieldTmpNo).bind('click', function(){
    //console.log($('#txtAddFieldTag_'+fieldTmpNo).val());
    var tagName = $('#txtAddFieldTag_'+fieldTmpNo).val();
    addFieldAddSubfieldEditor(jQRowGroupID, fieldTmpNo, "", "", tagName);
  });
  ...
  $('#selectAddFieldTemplate_' + fieldTmpNo).bind('change', function(e){
      ...
      //console.log(jQRowGroupID + " == " + fieldTemplates[value] + " == " + fieldTmpNo );
...
//TP: aqdded function
function onSelectAddValueChanged(e, tag, fieldPosition, subfieldPosition){
//  alert(e);
  // enter key pressed.
  var subfieldsNum = $('#rowGroup_' + tag + '_' + fieldPosition + ' .bibEditTxtSubfieldCode').length;
  if (subfieldPosition < (subfieldsNum - 1)){
...
//  console.log($('#txtAddFieldTag_'+fieldTmpNo).val());
  if (e.which == 13){
    var f = $('#txtAddFieldTag_'+fieldTmpNo).val();
    if (f != '190' && f != '500' && f != '700' && f != '400') {    
      // enter key pressed
      ...
      var sfq = false;
      var sfg = false;
      $(jQRowGroupID+' .bibEditTxtSubfieldCode').each(function() {
        switch ($(this).val()) {
          case 'a': sfa=true;break;
          case '1': sf1=true;break;
          case 'a': sfb=true;break;
          case '3': sf3=true;break;
          case '4': sf4=true;break;
          case 'q': sfq=true;break;
          case 'g': sfg=true;break;
        }
      });
      if (sfg && (sfa || sf1 || sfb || sf3 || sf4 || sfq)) {
        alert('!!! nesmí být vyplněno zároveň podpole g a některé z podpolí a, 1, b, 3, 4, q ');
        return false;
      } else {
        if (sfa && sf1) {$('#txtAddFieldInd1_'+fieldTmpNo).val('1');}
        else if (sfa && !sf1) {$('#txtAddFieldInd1_'+fieldTmpNo).val('0');}
        else if (!sfa && sf1) {$('#txtAddFieldInd1_'+fieldTmpNo).val('1');}
        if (sfg) {$('#txtAddFieldInd2_'+fieldTmpNo).val('1');}
        else {$('#txtAddFieldInd2_'+fieldTmpNo).val('0');}
      }
      onAddFieldJumpToNextSubfield(jQRowGroupID, fieldTmpNo, subfieldInd);
    }
  }
...
function onAddFieldSave(event){
  var fieldTmpNo = this.id.split('_')[1];
  addFieldSave(fieldTmpNo);
//  TP: add selectBoxes after field saved (not submited)
}
    ...
    function(value){
      ...
      data: function(){
        ...
        if (tmpArray[0] == 'fieldTag'){
          ...
          console.log('!11 '+tmpResult);
        }
        else {
            tmpResult = field[0][subfieldIndex][1];
          console.log('!12 '+tmpResult);
        }
...
function drawSeelectSubfieldValues() {
  /*
   * selectbox for existing subfield content
   */
  $('#bibEditContent tr').each(function() {
    ...
    if (tmpArray[3] != undefined) {
      ...
      var subfield = field[0][subfieldIndex][0];
      var subfieldValues = getSubfieldValues(tag.toString()+subfield.toString());
      var newId = 'contentSelectBox_'+tag+'_'+fieldPosition+'_'+subfieldIndex;
//      console.log(subfieldValues);
      if (subfieldValues !== undefined) {
        if ($("#"+newId).length==0) {
//        var selectbox = '<tr class="selectbox_ext"><td></td><td></td><td></td><td></td><td id="contentSelectBox_'+tag+'_'+fieldPosition+'_'+subfieldIndex+'" class="value_selectbox_wrapper_external">'+select('selectAddValue_' + tag+'_'+fieldPosition+'_'+subfieldIndex, subfieldValues, '0') + '</td><td></td>';
//alert("testiiik");
          var selectbox = '<tr class="selectbox_ext"><td></td><td></td><td></td><td></td><td id="'+newId+'" class="value_selectbox_wrapper_external">'+select('selectAddValue_' + tag+'_'+fieldPosition+'_'+subfieldIndex, subfieldValues, '0') + '</td><td></td>';
          $(this).before(selectbox);
        }
      }
    }
...
function drawField_856_ImagesThumbs() {
  $('#bibEditTable tbody').each(function() {
    var tempId = this.id;
    //console.log(tempId.substr(9, 3));
    if (tempId.substr(9, 3) == '956') {
      $(this).find('.bibEditCellContent').each(function() {
//        alert($(this).text());
        if ($(this).text().substr(0, 6) == 'HUAVCR') {
          if ($("[src='/datastorage/"+$(this).text()+"']").length==0) {
//            $('#bibEditContentTable')
//              .append("<a href='/datastorage/" + $(this).text() + "' title='' onClick='return false;' class='imgLink' style='display:inline-block;'><img src='/datastorage/" + $(this).text() + "' height='100' /></a>") //TP: toto predelat, at je datastorage dir definovan jako konstanta v conf
            draw_ImagesThumbs($(this).text(),'');
  ...
  $('.imgLink').each(function() {
    cUrl = $(this).attr('title');
//    alert(cUrl+": "+$('td[id^="content_956"]:contains("kuk")'));
//    alert(cUrl+": "+$('td:contains("kuk")').);
    cTd = $('#bibEditTable td').filter(function() {
...
function adNewImageField(url,label) { //TP: 
  createAddFieldInterface();
  var currTmpNo=(onAddFieldClick.addFieldFreeTmpNo-1);
  $('#txtAddFieldTag_'+currTmpNo).attr('value', '956');
  $('#txtAddFieldInd1_'+currTmpNo).attr('value', '4');
  $('#txtAddFieldInd2_'+currTmpNo).attr('value', '');
  $('#txtAddFieldSubfieldCode_'+currTmpNo+'_0').attr('value', 'u');
  $('#txtAddFieldValue_'+currTmpNo+'_0').attr('value', url);
  var tagName = $('#txtAddFieldTag_'+currTmpNo).val();
  var jQRowGroupID = '#rowGroupAddField_' + currTmpNo;
  addFieldAddSubfieldEditor(jQRowGroupID, currTmpNo, "y", label, tagName);
  addFieldSave(currTmpNo);
}
...
function proceedToConfirm() { //TP:
//  alert("test3");
//  $('#selectAddValue_' + fieldTmpNo + '_' + subfieldTmpNo).on("change",function(e) {changeTxtAddFieldValue('#selectAddValue_' + fieldTmpNo + '_' + subfieldTmpNo,e.value);});//TP: addded line
