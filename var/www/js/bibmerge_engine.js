...
function initStateFromHash() {
  ...
  if (gHash == '') {
//    $('#bibMergeContent').html('Select two records to be compared from the side panel.'); TP:bylo puvodne
    var recid = $('#bibMergeRecInput1').attr('value');
    if (recid != '') {
      $('#bibMergeContent').html('Changes have been submitted.<br/><input type="button" class="bibMergeRecordProceedAfterMerge" value="Update master record"/><br/>'); 
    } else {
...
function initContent() {
  ...
  $('.bibMergeRecordProceedAfterMerge').live('click', onclickRecordProceedAfterMerge); //TP: added
...
function onclickSubmitButton() {
  ...
  if (displayAlert('confirmSubmit')){
      var _data = {
//        requestType: 'submit', TP: puvodni
        requestType: 'mergeRecord',
        recID1: gRecID1
      };
      if (checkbox == true)
