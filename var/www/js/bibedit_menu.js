...
function initMenu(){
  ...
  $('#btnNextCardImages').bind('click', onNextCardImagesClick); //TP: added
...
function updateStatus(statusType, reporttext, enableToolbar){
  ...
  $('#btnNextCardImages').attr('disabled', 'disabled');  //TP: disable running card checking button
  switch (statusType){
    case 'ready':
      image = img('/img/circle_green.png');
      text = 'Ready';
      drawSeelectSubfieldValues(); //TP: draw the value selection
      drawField_856_ImagesThumbs(); //TP: draw the thumbs
      $('#btnNextCardImages').removeAttr('disabled'); //TP: enable running card checking button
      break;
    case 'updating':
